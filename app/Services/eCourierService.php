...
class eCourierService
{
    ...
    {
        $this->baseUrl = config('services.ecourier.base_url', 'https://ecourier.com.bd');
        $this->apiKey = config('services.ecourier.api_key');
        $this->secretKey = config('services.ecourier.secret_key');
        $this->userId = config('services.ecourier.user_id');
    }

    /**
     * Create order in eCourier system
     */
    public function createOrder(Order $order)
    {
        try {
            ...
            ])->post($this->baseUrl . '/api/v1/order/create', $payload);
            ...
            if ($response->successful() && isset($responseData['tracking_code'])) {
                ...
                $order->update([
                    'notes' => 'eCourier: ' . $responseData['tracking_code'] . ' (Consignment: ' . ($responseData['consignment_id'] ?? 'N/A') . ')'
                ]);
                ...
                return [
                    ...
                    'tracking_number' => $responseData['tracking_code'],
                    'consignment_id' => $responseData['consignment_id'] ?? null,
                    'message' => 'Order successfully created in eCourier system'
                ];
            }

            return [
                'success' => false,
                'message' => $responseData['message'] ?? 'Failed to create order in eCourier',
                'errors' => $responseData
            ];

        } catch (\Exception $e) {
            Log::error('eCourier API Error: ' . $e->getMessage());
    ...
    public function trackOrder($trackingCode)
    {
        try {
            $response = Http::withHeaders([
                ...
                'SECRET-KEY' => $this->secretKey,
                'USER-ID' => $this->userId,
            ])->get($this->baseUrl . '/api/v1/order/track/' . $trackingCode);

            return $response->json();

        } catch (\Exception $e) {
            Log::error('eCourier Tracking Error: ' . $e->getMessage());
            return null;
        }
    }
    ...
    public function cancelOrder($consignmentId)
    {
        try {
            $response = Http::withHeaders([
                ...
                'USER-ID' => $this->userId,
                'Content-Type' => 'application/json',
            ])->post($this->baseUrl . '/api/v1/order/cancel', [
            ...
            if ($response->successful()) {
                ...
                CourierIntegration::where('courier_type', 'ecourier')
            ...
            Log::error('eCourier Cancel Error: ' . $e->getMessage());
            return [
                'success' => false,
                'message' => 'System error: ' . $e->getMessage()
            ];
    ...
    public function getDeliveryAreas()
    {
        try {
            $response = Http::withHeaders([
                ...
                'SECRET-KEY' => $this->secretKey,
                'USER-ID' => $this->userId,
            ])->get($this->baseUrl . '/api/v1/delivery-areas');

            return $response->json();

        } catch (\Exception $e) {
            Log::error('eCourier Areas Error: ' . $e->getMessage());
            return null;
        }
    }
    ...
    public function calculateDeliveryCharge($data)
    {
        try {
            $response = Http::withHeaders([
                ...
                'USER-ID' => $this->userId,
                'Content-Type' => 'application/json',
            ])->post($this->baseUrl . '/api/v1/calculate-charge', $data);

            return $response->json();

        } catch (\Exception $e) {
            Log::error('eCourier Charge Calculation Error: ' . $e->getMessage());
            return null;
        }
    }

    /**
     * Prepare order payload for eCourier API
     */
    ...
    {
        return config('services.ecourier.pickup_address', 'Shop Address, Dhaka');
    }
