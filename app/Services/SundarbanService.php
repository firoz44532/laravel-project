...
class SundarbanService
{
    ...
    {
        $this->baseUrl = config('services.sundarban.base_url', 'https://sundarban.com');
        $this->apiKey = config('services.sundarban.api_key');
        $this->merchantId = config('services.sundarban.merchant_id');
    }

    /**
     * Create order in Sundarban system
     */
    public function createOrder(Order $order)
    {
        try {
            ...
            if ($response->successful() && isset($responseData['tracking_code'])) {
                ...
                $order->update([
                    'notes' => 'Sundarban: ' . $responseData['tracking_code'] . ' (Consignment: ' . ($responseData['consignment_id'] ?? 'N/A') . ')'
                ]);
                ...
                return [
                    ...
                    'tracking_number' => $responseData['tracking_code'],
                    'consignment_id' => $responseData['consignment_id'] ?? null,
                    'message' => 'Order successfully created in Sundarban system'
                ];
            }

            return [
                'success' => false,
                'message' => $responseData['message'] ?? 'Failed to create order in Sundarban',
                'errors' => $responseData
            ];

        } catch (\Exception $e) {
            Log::error('Sundarban API Error: ' . $e->getMessage());
    ...
    public function trackOrder($trackingCode)
    {
        try {
            ...
            Log::error('Sundarban Tracking Error: ' . $e->getMessage());
            return null;
        }
    }
    ...
    public function cancelOrder($consignmentId)
    {
        try {
            ...
            if ($response->successful()) {
                ...
                CourierIntegration::where('courier_type', 'sundarban')
            ...
            Log::error('Sundarban Cancel Error: ' . $e->getMessage());
            return [
                'success' => false,
                'message' => 'System error: ' . $e->getMessage()
            ];
    ...
    /**
     * Prepare order payload for Sundarban API
     */
    ...
    {
        return config('services.sundarban.pickup_address', 'Shop Address, Dhaka');
    }
